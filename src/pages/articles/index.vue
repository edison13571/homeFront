  ...
  <div class="main">
    ...
    <div class="memoryDate-wrap">
      <div class="memoryDate-tabs">
        <div :class="type==='year'?'memoryDate-tabs-unit-active':'memoryDate-tabs-unit'" @click="typeChange('year')">列表</div>
      ...
      <div class="memoryDate-main">
        <div class="memoryDate-wrap-unit" v-for="(item,index) in urls" :key="index" @click="memoryDateClick(item)"
          ...
          <div class="memoryDate-wrap-unit-logo">{{item.title.charAt(0)}}</div>
        </div>
      </div>
    </div>
...
<script>
  ...
  import memoryDateAdd from "./Add"
  import memoryDateEdit from "./Edit"
    ...
    methods: {
      tempShow:function () {
        if(this.tempName){
          ...
          return `${item.title} views:${item.views}／显示:${item.publicAble}`
        }
        return ""
      },
      searchLocal() {
