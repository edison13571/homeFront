<template>
  <div class="memoryDate-add">
    <div class="memoryDate-add-wrap">
      <div class="memoryDate-add-title">修改记录</div>
...
<script>
import {memoryDateEditInfo} from "../../../api";
...
export default {
    ...
    beforeMount() {
      ...
      this.name=info.name;
      this.recallDate=info.recallDate;
      this.recallMonth=info.recallMonth;
      this.theme=info.theme;
      this.recallYear=info.recallYear;
      this.type=info.type;
      this.id=info._id;
    },
    methods:{
      ...
      submit(){
        ...
        memoryDateEditInfo(data).then(res=>{
          if (res.success){
            this.$emit('finish')
            this.$emit('close')
          } else {
